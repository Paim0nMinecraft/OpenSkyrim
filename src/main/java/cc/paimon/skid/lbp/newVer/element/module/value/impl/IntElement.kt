...
class IntElement(val savedValue: IntegerValue): ValueElement<Int>(savedValue) {
    ...
    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, bgColor: Color, accentColor: Color): Float {
        val valueDisplay = 30F + Fonts.posterama40.getStringWidth("${savedValue.maximum}")
        ...
        val currLength = Fonts.posterama40.getStringWidth("${savedValue.get()}")
        ...
        Fonts.posterama40.drawString("${savedValue.get()}", x + width + 6F - valueDisplay, y + 10F - Fonts.posterama40.fontHeight / 2F + 2F, Color(26, 26, 26).getRGB())
