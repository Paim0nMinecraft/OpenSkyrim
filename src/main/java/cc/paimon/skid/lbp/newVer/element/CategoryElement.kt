...
class CategoryElement(val category: ModuleCategory): MinecraftInstance() {
    ...
    fun drawLabel(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, height: Float) {
        if (focused)
            ...
            RenderUtils.drawRoundedRect(x + 3F, y + 3F, x + width - 3F, y + height - 3F, 3F, Color(241, 243, 247).rgb)
        Fonts.posterama40.drawString(name, x + 10F, y + height / 2F - Fonts.posterama40.fontHeight / 2F + 2F, Color(26, 26, 26).getRGB())
    }
    ...
    fun drawPanel(mX: Int, mY: Int, x: Float, y: Float, width: Float, height: Float, wheel: Int, accentColor: Color) {
        ...
        Fonts.posterama35.drawString("${ChatFormatting.GRAY}Modules > ${ChatFormatting.RESET}$name", x + 10F, y + 10F, Color(26, 26, 26).getRGB())
        Fonts.posterama20.drawString("$name", x - 190F, y - 12F, Color(26, 26, 26).getRGB())
