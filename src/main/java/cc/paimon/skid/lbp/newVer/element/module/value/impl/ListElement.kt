...
class ListElement(val saveValue: ListValue): ValueElement<String>(saveValue) {
    ...
    private val maxSubWidth = -(saveValue.values.map { -Fonts.posterama40.getStringWidth(it) }.sorted().firstOrNull() ?: 0) + 20

    companion object {
        val expanding = LiquidBounce.wrapper.classProvider.createResourceLocation("paimon/expand.png") }
    ...
    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, bgColor: Color, accentColor: Color): Float {
       ...
       Fonts.posterama40.drawString(value.name, x + 10F, y + 10F - Fonts.posterama40.fontHeight / 2F + 2F, Color(26, 26, 26).getRGB())
        ...
        glPopMatrix()
        glPopMatrix()
       Fonts.posterama40.drawString(value.get(), x + width - 14F - maxSubWidth, y + 6F, Color(26, 26, 26).getRGB())
        glPushMatrix()
        ...
        if (percent > 0F) for (subV in unusedValues) {
           Fonts.posterama40.drawString(subV, 0F, (16F + vertHeight) * percent - 1F, Color(.5F, .5F, .5F, percent.coerceIn(0F, 1F)).rgb)
            vertHeight += 16F
        }
        glPopMatrix()
