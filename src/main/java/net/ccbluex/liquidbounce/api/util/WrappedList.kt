...
open class WrappedList<O, T, C : List<O>>(wrapped: C, unwrapper: (T) -> O, wrapper: (O) -> T) :
    WrappedCollection<O, T, C>(wrapped, unwrapper, wrapper), List<T> {
    ...
    override fun listIterator(index: Int): ListIterator<T> =
        WrappedCollectionIterator(wrapped.listIterator(index), wrapper)

    override fun subList(fromIndex: Int, toIndex: Int): List<T> =
        WrappedList(wrapped.subList(fromIndex, toIndex), unwrapper, wrapper)
