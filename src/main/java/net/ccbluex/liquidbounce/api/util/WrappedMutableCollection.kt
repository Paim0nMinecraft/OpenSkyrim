...
open class WrappedMutableCollection<O, T, C : MutableCollection<O>>(
    wrapped: C,
    unwrapper: (T) -> O,
    wrapper: (O) -> T
) : WrappedCollection<O, T, C>(wrapped, unwrapper, wrapper), MutableCollection<T> {
    ...
    class WrappedCollectionIterator<O, T>(val wrapped: MutableIterator<O>, val unwrapper: (O) -> T) :
        MutableIterator<T> {
