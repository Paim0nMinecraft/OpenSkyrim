...
class ChestStealer : Module() {
    ...
    fun onRender3D(event: Render3DEvent?) {
        ...
        if (!classProvider.isGuiChest(mc.currentScreen) || mc.currentScreen == null || !delayTimer.hasTimePassed(
                nextDelay
            )
        ) {
            ...
            if (takeRandomizedValue.get()) {
                do {
                    ...
                    for (slotIndex in 0 until screen.inventoryRows * 9) {
                        ...
                        if (stack != null && (!onlyItemsValue.get() || !classProvider.isItemBlock(stack.item)) && (!inventoryCleaner.state || inventoryCleaner.isUseful(
                                stack,
                                -1
                            ))
                        )
