...
class AntiBot : Module() {
    ...
    private val livingTimeValue = BoolValue("LivingTime", true)
    private val livingTimeTicksValue = IntegerValue("LivingTimeTicks", 16, 1, 200)
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (classProvider.isSPacketAnimation(packet)) {
            ...
            if (entity != null && classProvider.isEntityLivingBase(entity) && packetAnimation.animationType == 0 && !swing.contains(
                    entity.entityId
                )
    ...
    fun onAttack(e: AttackEvent) {
        ...
        if (entity != null && classProvider.isEntityLivingBase(entity) && !hitted.contains(entity.entityId)) hitted.add(
            entity.entityId
        )
    }
    ...
    companion object {
        ...
        fun isBot(entity: IEntityLivingBase): Boolean {
            ...
            if (antiBot.colorValue.get() && !entity.displayName!!.formattedText.replace("§r", "")
                    .contains("§")
            ) return true
            ...
            if (antiBot.duplicateInTabValue.get()) {
                if (mc.netHandler.playerInfoMap.stream()
                        .filter { networkPlayer: INetworkPlayerInfo? ->
                            entity.name == stripColor(
                                EntityUtils.getName(
                                    networkPlayer
                                )
                            )
