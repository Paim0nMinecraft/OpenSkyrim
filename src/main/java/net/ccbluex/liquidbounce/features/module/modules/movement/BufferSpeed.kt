...
class BufferSpeed : Module() {
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if (thePlayer.onGround) {
            ...
            if (slimeValue.get() && (classProvider.isBlockSlime(getBlock(blockPos.down())) || classProvider.isBlockSlime(
                    getBlock(blockPos)
                ))
            ) {
                thePlayer.jump()
            ...
            if (stairsValue.get() && (classProvider.isBlockStairs(getBlock(blockPos.down())) || classProvider.isBlockStairs(
                    getBlock(blockPos)
                ))
            ) {
            ...
            if (iceValue.get() && (getBlock(blockPos.down()) == classProvider.getBlockEnum(BlockType.ICE) || getBlock(
                    blockPos.down()
                ) == classProvider.getBlockEnum(BlockType.ICE_PACKED))
            ) {
            ...
            if (wallValue.get()) {
                when (wallModeValue.get().toLowerCase()) {
                    "old" -> if (thePlayer.isCollidedVertically && isNearBlock || !classProvider.isBlockAir(
                            getBlock(
