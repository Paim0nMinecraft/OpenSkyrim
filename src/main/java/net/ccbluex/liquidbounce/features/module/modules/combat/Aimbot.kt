...
class Aimbot : Module() {
    ...
    fun onStrafe(event: StrafeEvent) {
        ...
        if (jitterValue.get()) {
            ...
            if (yaw)
                thePlayer.rotationYaw += if (yawNegative) -RandomUtils.nextFloat(0F, 1F) else RandomUtils.nextFloat(
                    0F,
                    1F
                )

            if (pitch) {
                thePlayer.rotationPitch += if (pitchNegative) -RandomUtils.nextFloat(0F, 1F) else RandomUtils.nextFloat(
                    0F,
                    1F
                )
