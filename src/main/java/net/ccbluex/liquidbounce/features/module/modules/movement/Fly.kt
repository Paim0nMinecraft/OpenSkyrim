...
class Fly : Module() {
    ...
    override fun onDisable() {
        ...
        val x = -sin(yaw) * 0.2f
        val z = cos(yaw) * 0.2f
        ClientUtils.displayChatMessage(x.toString())
        ClientUtils.displayChatMessage(yaw.toString())

        ClientUtils.displayChatMessage(z.toString())
        mc.netHandler.addToSendQueue(
            classProvider.createCPacketPlayerPosition(
                mc.thePlayer!!.posX + x,
...
//                    mc.effectRenderer.spawnEffectParticle(EnumParticleTypes.FLAME.getParticleID(), thePlayer.posX, thePlayer.posY + 0.2, thePlayer.posZ, -thePlayer.motionX, -0.5, -thePlayer.motionZ)
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (classProvider.isSPacketPlayerPosLook(event.packet)) {
            ...
            if (mode.equals("Vanilla", ignoreCase = true)) {
//               event.cancelEvent()
//                ClientUtils.displayChatMessage("sp")
            }
        }
    }
    ...
    fun onBB(event: BlockBBEvent) {
        ...
        val mode = modeValue.get()
//        if classProvider.isBlockAir(event.block) && event.y <= fly.launchY) {
