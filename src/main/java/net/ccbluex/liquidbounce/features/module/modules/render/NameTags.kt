...
import net.ccbluex.liquidbounce.ui.cape.GuiCapeManager.drawTexturedModalRect
...
@ModuleInfo(name = "NameTags", description = "0231 turn it into 1.12.2", category = ModuleCategory.RENDER)
class NameTags : Module() {
    ...
    val nfpValue = BoolValue("NoFirstPerson", true)
    ...
    private fun renderNameTag(entity: IEntityLivingBase, tag: String) {
        ...
        if (borderValue.get())
            quickDrawBorderedRect(
                ...
                -2F,
                width + 4F,
                fontRenderer.fontHeight + 2F + if (healthBarValue.get()) 2F else 0F,
                2F,
                borderColor.rgb,
                bgColor.rgb
            )
        ...
        if (potionValue.get() && entity is EntityPlayer) {
            val potions = (entity.getActivePotionEffects() as Collection<PotionEffect>).map { Potion.getPotionById(0) }
            ...
            if (!potions.isEmpty()) {
                ...
                for (potion in potions) {
                    ...
                    drawTexturedModalRect(minX + index * 20, -22, 0 + i1 % 8 * 18, 198 + i1 / 8 * 18, 18, 18)
                    index++
                }
