...
//        this.prevGui = prevGui;
    ...
    public void initGui() {
        ...
        searchField = classProvider.createGuiTextField(2, Fonts.posterama40, representedScreen.getWidth() - textFieldWidth - 10, 10, textFieldWidth, 20);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.posterama40.drawCenteredString("AltManager", representedScreen.getWidth() / 2.0f, 6, 0xffffff);
        Fonts.posterama35.drawCenteredString(this.searchField.getText().isEmpty() ? (LiquidBounce.fileManager.accountsConfig.getAccounts().size() + " Alts") : this.altsList.accounts.size() + " Search Results", representedScreen.getWidth() / 2.0f, 18, 0xffffff);
        Fonts.posterama35.drawCenteredString(status, representedScreen.getWidth() / 2.0f, 32, 0xffffff);
        //  Fonts.posterama35.drawStringWithShadow("§7User: §a"), 6, 6, 0xffffff);
        //  Fonts.posterama35.drawStringWithShadow("§7Type: §a" + (altService.getCurrentService() == AltService.EnumAltService.THEALTENING ? "TheAltening" : MCLeaks.isAltActive() ? "MCLeaks" : UserUtils.INSTANCE.isValidTokenOffline(mc.getSession().getToken()) ? "Premium" : "Cracked"), 6, 15, 0xffffff);

        this.searchField.drawTextBox();

        if (searchField.getText().isEmpty() && !searchField.isFocused())
            Fonts.posterama40.drawStringWithShadow("§7Search...", this.searchField.getXPosition() + 4, 17, 0xffffff);
    ...
    public void actionPerformed(IGuiButton button) throws IOException {
        ...
        switch (button.getId()) {
            case 0:
                mc2.displayGuiScreen(new GuiSoarMainMenu());
                break;
            case 1:
    ...
    private class GuiList extends WrappedGuiSlot {
        ...
        public void drawSlot(int id, int x, int y, int var4, int var5, int var6) {
            ...
            Fonts.posterama40.drawCenteredString(minecraftAccount.getAccountName() == null ? minecraftAccount.getName() : minecraftAccount.getAccountName(), (representedScreen.getWidth() / 2), y + 2, Color.WHITE.getRGB(), true);
