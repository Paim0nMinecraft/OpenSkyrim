...
class Slowly(inst: Target) : TargetStyle("Slowly", inst, true) {
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
        ...
        val length =
            60.coerceAtLeast(font.getStringWidth(entity.name!!)).coerceAtLeast(font.getStringWidth(healthString))
                .toFloat() + 10F
    ...
    override fun handleBlur(player: IEntityPlayer) {
        ...
        val length =
            60.coerceAtLeast(font.getStringWidth(player.name!!)).coerceAtLeast(font.getStringWidth(healthString))
                .toFloat() + 10F
    ...
    override fun getBorder(entity: IEntityLivingBase?): Border {
        entity ?: return Border(0F, 0F, 102F, 36F, 0f)
        val font = Fonts.minecraftFont
        val healthString = "${decimalFormat2.format(entity.health)} ❤"
        val length =
            60.coerceAtLeast(font.getStringWidth(entity.name!!)).coerceAtLeast(font.getStringWidth(healthString))
                .toFloat() + 10F
        return Border(0F, 0F, 32F + length, 36F, 0f)
    }
