...
class EditorPanel(private val hudDesigner: GuiHudDesigner, var x: Int, var y: Int) : MinecraftInstance() {
    ...
    private fun drawCreate(mouseX: Int, mouseY: Int) {
        ...
        for (element in elements) {
            ...
            Fonts.posterama35.drawString(name, x + 2.0f, y + height.toFloat(), Color.WHITE.rgb)

            val stringWidth = Fonts.posterama35.getStringWidth(name)
        ...
        Fonts.posterama35.drawString("§lCreate element", x + 2F, y + 3.5F, Color.WHITE.rgb)
    }
    ...
    private fun drawSelection(mouseX: Int, mouseY: Int) {
        ...
        Fonts.posterama35.drawString("§lCreate element", x + 2.0f, y.toFloat() + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height
            && mouseY <= y + height + 10
        )
            create = true
        ...
        Fonts.posterama35.drawString("§lReset", x + 2.toFloat(), y.toFloat() + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height
            && mouseY <= y + height + 10
        )
        ...
        Fonts.posterama35.drawString("§lAvailable Elements", x + 2.0f, y + height.toFloat(), Color.WHITE.rgb)
        height += 10
        realHeight += 10

        for (element in LiquidBounce.hud.elements) {
            Fonts.posterama35.drawString(element.name, x + 2, y + height, Color.WHITE.rgb)

            val stringWidth = Fonts.posterama35.getStringWidth(element.name)
            if (width < stringWidth + 8)
                width = stringWidth + 8
        ...
        Fonts.posterama35.drawString("§lEditor", x + 2F, y + 3.5f, Color.WHITE.rgb)
    }
    ...
    private fun drawEditor(mouseX: Int, mouseY: Int) {
        ...
        // Scale
        Fonts.posterama35.drawString("Scale: ${"%.2f".format(element.scale)}", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Horizontal
        Fonts.posterama35.drawString("H:", x + 2, y + height, Color.WHITE.rgb)
        Fonts.posterama35.drawString(
        ...
        // Vertical
        Fonts.posterama35.drawString("V:", x + 2, y + height, Color.WHITE.rgb)
        Fonts.posterama35.drawString(
        ...
        for (value in element.values) {
            when (value) {
                is BoolValue -> {
                    ...
                    val stringWidth = Fonts.posterama35.getStringWidth(value.name)
                    if (width < stringWidth + 8)
                        width = stringWidth + 8
                ...
                is FloatValue -> {
                    ...
                    Fonts.posterama35.drawString(text, x + 2, y + height, Color.WHITE.rgb)

                    val stringWidth = Fonts.posterama35.getStringWidth(text)
                    if (width < stringWidth + 8)
                        width = stringWidth + 8
                ...
                is ListValue -> {
                    // Title
                    Fonts.posterama35.drawString(value.name, x + 2, y + height, Color.WHITE.rgb)
        ...
        // Header
        RenderUtils.drawRect(x, y, x + width, y + 12, ClickGUI.generateColor().rgb)
        Fonts.posterama35.drawString("§l${element.name}", x + 2F, y + 3.5F, Color.WHITE.rgb)

        // Delete button
        if (!element.info.force) {
            val deleteWidth = x + width - Fonts.posterama35.getStringWidth("§lDelete") - 2F
            Fonts.posterama35.drawString("§lDelete", deleteWidth, y + 3.5F, Color.WHITE.rgb)
