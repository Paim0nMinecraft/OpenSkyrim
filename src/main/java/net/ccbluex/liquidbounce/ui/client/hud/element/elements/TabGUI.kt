...
class TabGUI(x: Double = 5.0, y: Double = 25.0) : Element(x = x, y = y) {
    ...
    private val fontValue = FontValue("Font", Fonts.posterama35)
    ...
    init {
        for (category in ModuleCategory.values()) {
            ...
            tabs.add(tab)
            if (tabs.size > 5) tabs.removeAt(tabs.size - 1)
        }
    }
    ...
    override fun drawElement(): Border {
        ...
        return Border(1F, 0F, width.get(), guiHeight, 0F)
    }
        ...
        fun drawTab(
            ...
            for (module in modules)
                if (fontRenderer.getStringWidth(if (upperCase) module.name.toUpperCase() else module.name) + 4 > maxWidth)
                    maxWidth =
                        (fontRenderer.getStringWidth(if (upperCase) module.name.toUpperCase() else module.name) + 7F).toInt()
            ...
            RenderUtils.drawRect(
                x - 1.toFloat(), y + itemY + 1, x + 1f, y + itemY + 9, RenderUtils.getGradientOffset(
                    Color(hud.r.get(), hud.g.get(), hud.b.get()),
                    Color(hud.r2.get(), hud.g2.get(), hud.b2.get()),
                    (Math.abs(
                        System.currentTimeMillis() / hud.gradientSpeed.get()
            ...
            modules.forEachIndexed { index, module ->
                ...
                fontRenderer.drawString(
                    if (upperCase) module.name.toUpperCase() else module.name, x + 4F,
