...
class Chill(inst: Target) : TargetStyle("Chill", inst, true) {
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
        ...
        val tWidth = (45F + Fonts.posterama40.getStringWidth(name!!)
            .coerceAtLeast(Fonts.posterama72.getStringWidth(decimalFormat.format(health)))).coerceAtLeast(120F)
        ...
        // name + health
        Fonts.posterama40.drawString(name, 38F, 6F, getColor(-1).rgb)
    ...
    override fun handleBlur(player: IEntityPlayer) {
        val tWidth = (45F + Fonts.posterama40.getStringWidth(player.name!!)
            .coerceAtLeast(Fonts.posterama72.getStringWidth(decimalFormat.format(player.health)))).coerceAtLeast(120F)
    ...
    override fun handleShadow(player: IEntityPlayer) {
        val tWidth = (45F + Fonts.posterama40.getStringWidth(player.name!!)
            .coerceAtLeast(Fonts.posterama72.getStringWidth(decimalFormat.format(player.health)))).coerceAtLeast(120F)
        RenderUtils.drawRoundedRect(0F, 0F, tWidth, 48F, 7f, shadowOpaque.rgb)
    }

    override fun getBorder(entity: IEntityLivingBase?): Border {
        entity ?: return Border(0F, 0F, 120F, 48F, 0f)
        val tWidth = (45F + Fonts.posterama40.getStringWidth(entity.name!!)
            .coerceAtLeast(Fonts.posterama72.getStringWidth(decimalFormat.format(entity.health)))).coerceAtLeast(120F)
        return Border(0F, 0F, tWidth, 48F, 0f)
    }
