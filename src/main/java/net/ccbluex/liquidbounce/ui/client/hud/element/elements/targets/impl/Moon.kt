...
class Moon(inst: Target) : TargetStyle("Moon", inst, true) {
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
        ...
        RenderUtils.drawRect(
            53F,
            41f,
            17.5F + (easingHealth / entity.maxHealth).coerceIn(0F, 1F) * 138F,
            48.5F,
        ...
        Fonts.posterama35.drawString("Name: " + entity.name, 53.5f, 3.5f, getColor(-1).rgb)
        ...
        Fonts.posterama35.drawString(
            "Health: ${(decimalFormat.format(entity.health))}",
            53.5f,
            22f,
            targetInstance.barColor.rgb
        )
        ...
        if (playerInfo != null) {
            Fonts.posterama35.drawString(
                "Ping: ${playerInfo.responseTime.coerceAtLeast(0)}" + "ms",
    ...
    override fun getBorder(entity: IEntityLivingBase?): Border {
        entity ?: return Border(0F, 0F, 118F + 40.5f, 51.5F, 0f)
        val width = (38 + Fonts.posterama40.getStringWidth(entity.name!!))
            .coerceAtLeast(118)
            .toFloat()
        return Border(0F, 0F, width + 40.5f, 51.5F, 0f)
    }
