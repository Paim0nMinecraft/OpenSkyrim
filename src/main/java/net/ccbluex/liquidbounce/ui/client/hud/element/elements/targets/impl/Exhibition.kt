...
class Exhibition(inst: Target) : TargetStyle("Exhibition", inst, true) {
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
        ...
        for (index in 3 downTo 0) {
            val stack = entity.asEntityPlayer().inventory.armorInventory[index] ?: continue
        ...
        val mainStack = entity.asEntityPlayer().heldItem
    ...
    override fun handleBlur(player: IEntityPlayer) {
        val font = Fonts.fontTahoma
        val minWidth = 143F.coerceAtLeast(47F + font.getStringWidth(player.name!!))
        GlStateManager.enableBlend()
    ...
    override fun getBorder(entity: IEntityLivingBase?): Border {
        entity ?: return Border(-3F, -3F, 143F, 48F, 0f)

        val font = Fonts.fontTahoma
        val minWidth = 143F.coerceAtLeast(47F + font.getStringWidth(entity.name!!))

        return Border(-3F, -3F, minWidth, 48F, 0f)
    }
