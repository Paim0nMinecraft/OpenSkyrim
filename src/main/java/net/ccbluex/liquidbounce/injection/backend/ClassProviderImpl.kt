...
object ClassProviderImpl : IClassProvider {
    ...
    override fun createResourceLocation(resourceName: String): IResourceLocation =
        ResourceLocationImpl(ResourceLocation(resourceName))

    override fun createThreadDownloadImageData(
        cacheFileIn: File?,
        imageUrlIn: String,
        textureResourceLocation: IResourceLocation?,
        imageBufferIn: WIImageBuffer
    ): IThreadDownloadImageData {
        return ThreadDownloadImageDataImpl(
            ThreadDownloadImageData(
                cacheFileIn,
                imageUrlIn,
                textureResourceLocation?.unwrap(),
                object : IImageBuffer {
                    override fun parseUserSkin(image: BufferedImage?): BufferedImage? =
                        imageBufferIn.parseUserSkin(image)
    ...
    override fun createClickEvent(action: IClickEvent.WAction, value: String): IClickEvent =
        ClickEventImpl(ClickEvent(action.unwrap(), value))

    override fun createGuiTextField(
        id: Int,
        ...
        width: Int,
        height: Int
    ): IGuiTextField = GuiTextFieldImpl(GuiTextField(id, iFontRenderer.unwrap(), x, y, width, height))

    override fun createGuiPasswordField(
        id: Int,
        iFontRenderer: IFontRenderer,
        x: Int,
        y: Int,
    ...
    override fun createItemStack(item: IItem, amount: Int, meta: Int): IItemStack =
        ItemStackImpl(ItemStack(item.unwrap(), amount, meta))

    override fun createEntityLightningBolt(
        world: IWorld,
        posX: Double,
        posY: Double,
        posZ: Double,
        effectOnly: Boolean
    ): IEntity = EntityLightningBolt(world.unwrap(), posX, posY, posZ, effectOnly).wrap()
    ...
    override fun createAxisAlignedBB(
        minX: Double,
        minY: Double,
        minZ: Double,
        maxX: Double,
        maxY: Double,
        maxZ: Double
    ): IAxisAlignedBB = AxisAlignedBBImpl(AxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ))

    override fun createScaledResolution(mc: IMinecraft): IScaledResolution =
        ScaledResolutionImpl(ScaledResolution(mc.unwrap()))
    ...
    override fun createEntityOtherPlayerMP(world: IWorldClient, gameProfile: GameProfile): IEntityOtherPlayerMP =
        EntityOtherPlayerMPImpl(EntityOtherPlayerMP(world.unwrap(), gameProfile))

    override fun createPotionEffect(id: Int, time: Int, strength: Int): IPotionEffect =
        PotionEffectImpl(PotionEffect(Potion.getPotionById(id), time, strength))

    override fun createGuiOptions(parentScreen: IGuiScreen, gameSettings: IGameSettings): IGuiScreen =
        GuiScreenImpl(GuiOptions(parentScreen.unwrap(), gameSettings.unwrap()))

    override fun createGuiSelectWorld(parentScreen: IGuiScreen): IGuiScreen =
        GuiScreenImpl(GuiWorldSelection(parentScreen.unwrap()))

    override fun createGuiMultiplayer(parentScreen: IGuiScreen): IGuiScreen =
        GuiScreenImpl(GuiMultiplayer(parentScreen.unwrap()))

    override fun createGuiModList(parentScreen: IGuiScreen): IGuiScreen =
        GuiScreenImpl(GuiModList(parentScreen.unwrap()))

    override fun createGuiConnecting(parent: IGuiScreen, mc: IMinecraft, serverData: IServerData): IGuiScreen =
        GuiScreenImpl(GuiConnecting(parent.unwrap(), mc.unwrap(), serverData.unwrap()))

    override fun createCPacketHeldItemChange(slot: Int): ICPacketHeldItemChange =
        CPacketHeldItemChangeImpl(CPacketHeldItemChange(slot))

    override fun createCPacketPlayerBlockPlacement(stack: IItemStack?): ICPacketPlayerBlockPlacement =
        Backend.BACKEND_UNSUPPORTED()

    override fun createCPacketPlayerBlockPlacement(
        positionIn: WBlockPos,
        placedBlockDirectionIn: Int,
        stackIn: IItemStack?,
        facingXIn: Float,
        facingYIn: Float,
        facingZIn: Float
    ): ICPacketPlayerBlockPlacement = CPacketPlayerBlockPlacementImpl(
        CPacketPlayerTryUseItemOnBlock(
            positionIn.unwrap(),
            EnumFacing.values()[placedBlockDirectionIn],
            EnumHand.MAIN_HAND,
            facingXIn,
            facingYIn,
            facingZIn
    ...
    override fun createCPacketPlayerPosLook(
        x: Double,
        y: Double,
        z: Double,
        yaw: Float,
        pitch: Float,
        onGround: Boolean
    ): ICPacketPlayerPosLook = CPacketPlayerPosLookImpl(CPacketPlayer.PositionRotation(x, y, z, yaw, pitch, onGround))

    override fun createCPacketClientStatus(state: ICPacketClientStatus.WEnumState): ICPacketClientStatus =
        CPacketClientStatusImpl(CPacketClientStatus(state.unwrap()))

    override fun createCPacketPlayerDigging(
        wAction: ICPacketPlayerDigging.WAction,
        pos: WBlockPos,
        facing: IEnumFacing
    ): IPacket = PacketImpl(CPacketPlayerDigging(wAction.unwrap(), pos.unwrap(), facing.unwrap()))

    override fun createCPacketTryUseItem(hand: WEnumHand): PacketImpl<*> =
        PacketImpl(CPacketPlayerTryUseItem(hand.unwrap()))

    override fun createCPacketPlayerPosition(x: Double, y: Double, z: Double, onGround: Boolean): ICPacketPlayer =
        CPacketPlayerImpl(CPacketPlayer.Position(x, y, z, onGround))

    override fun createICPacketResourcePackStatus(hash: String, status: ICPacketResourcePackStatus.WAction): IPacket =
        PacketImpl(CPacketResourcePackStatus(status.unwrap()))

    override fun createCPacketPlayerLook(yaw: Float, pitch: Float, onGround: Boolean): ICPacketPlayer =
        CPacketPlayerImpl(CPacketPlayer.Rotation(yaw, pitch, onGround))

    override fun createCPacketUseEntity(player: IEntity, wAction: ICPacketUseEntity.WAction): ICPacketUseEntity {
        return when (wAction) {
            ICPacketUseEntity.WAction.INTERACT -> CPacketUseEntityImpl(
                CPacketUseEntity(
                    player.unwrap(),
                    EnumHand.MAIN_HAND
                )
    ...
    override fun createCPacketUseEntity(entity: IEntity, positionVector: WVec3): ICPacketUseEntity =
        CPacketUseEntityImpl(CPacketUseEntity(entity.unwrap(), EnumHand.MAIN_HAND, positionVector.unwrap()))

    override fun createCPacketCreativeInventoryAction(slot: Int, itemStack: IItemStack): IPacket =
        PacketImpl(CPacketCreativeInventoryAction(slot, itemStack.unwrap()))

    override fun createCPacketEntityAction(
        player: IEntity,
        wAction: ICPacketEntityAction.WAction
    ): ICPacketEntityAction = CPacketEntityActionImpl(CPacketEntityAction(player.unwrap(), wAction.unwrap()))

    override fun createCPacketCustomPayload(channel: String, payload: IPacketBuffer): ICPacketCustomPayload =
        CPacketCustomPayloadImpl(CPacketCustomPayload(channel, payload.unwrap()))

    override fun createCPacketCloseWindow(windowId: Int): ICPacketCloseWindow =
        CPacketCloseWindowImpl(CPacketCloseWindow(windowId))
    ...
    override fun createCPacketAnimation(): ICPacketAnimation =
        CPacketAnimationImpl(CPacketAnimation(EnumHand.MAIN_HAND))
    ...
    override fun isTileEntityEnderChest(obj: Any?): Boolean =
        obj is TileEntityImpl && obj.wrapped is TileEntityEnderChest
    ...
    override fun isTileEntityShulkerBox(obj: Any?): Boolean =
        obj is TileEntityImpl && obj.wrapped is TileEntityShulkerBox
    ...
    override fun isSPacketResourcePackSend(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is SPacketResourcePackSend

    override fun isSPacketPlayerPosLook(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is SPacketPlayerPosLook
    ...
    override fun isSPacketEntityVelocity(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is SPacketEntityVelocity
    ...
    override fun isCPacketConfirmTransaction(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is CPacketConfirmTransaction


    override fun isCPacketPlayerBlockPlacement(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is CPacketPlayerTryUseItemOnBlock
    ...
    override fun isCPacketPlayerPosition(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is CPacketPlayer.Position

    override fun isCPacketPlayerPosLook(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is CPacketPlayer.PositionRotation
    ...
    override fun isCPacketHeldItemChange(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is CPacketHeldItemChange
    ...
    override fun isCPacketCustomPayload(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is CPacketCustomPayload

    override fun isCPacketHandshake(obj: Any?): Boolean = obj is PacketImpl<*> && obj.wrapped is C00Handshake

    override fun isCPacketPlayerDigging(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is CPacketPlayerDigging

    override fun isCPacketTryUseItem(obj: Any?): Boolean =
        obj is PacketImpl<*> && obj.wrapped is CPacketPlayerTryUseItem
    ...
    override fun isGuiHudDesigner(obj: Any?): Boolean =
        obj is GuiScreenImpl<*> && obj.wrapped is GuiScreenWrapper && obj.wrapped.wrapped is GuiHudDesigner

    override fun isClickGui(obj: Any?): Boolean =
        obj is GuiScreenImpl<*> && obj.wrapped is GuiScreenWrapper && obj.wrapped.wrapped is ClickGui
    ...
    override fun wrapFontRenderer(fontRenderer: IWrappedFontRenderer): IFontRenderer =
        FontRendererImpl(FontRendererWrapper(fontRenderer))
    ...
    override fun createSafeVertexBuffer(vertexFormat: IVertexFormat): IVertexBuffer =
        SafeVertexBuffer(vertexFormat.unwrap()).wrap()
    ...
    override fun wrapGuiSlot(
        wrappedGuiSlot: WrappedGuiSlot,
        mc: IMinecraft,
        width: Int,
        height: Int,
        top: Int,
    ...
    override fun createCPacketEncryptionResponse(
        secretKey: SecretKey,
        publicKey: PublicKey,
        verifyToken: ByteArray
    ): IPacket = PacketImpl(CPacketEncryptionResponse(secretKey, publicKey, verifyToken))
