...
class PlayerControllerMPImpl(val wrapped: PlayerControllerMP) : IPlayerControllerMP {
    ...
    override fun onPlayerRightClick(
        playerSP: IEntityPlayerSP,
        wWorld: IWorldClient,
        wItemStack: IItemStack?,
        wPosition: WBlockPos,
        wSideOpposite: IEnumFacing,
        wHitVec: WVec3
    ): Boolean {
        return wrapped.processRightClickBlock(
            playerSP.unwrap(),
            wWorld.unwrap(),
            wPosition.unwrap(),
            wSideOpposite.unwrap(),
            wHitVec.unwrap(),
            EnumHand.MAIN_HAND
    ...
    override fun clickBlock(blockPos: WBlockPos, enumFacing: IEnumFacing) =
        wrapped.clickBlock(blockPos.unwrap(), enumFacing.unwrap())

    override fun onPlayerDestroyBlock(blockPos: WBlockPos, enumFacing: IEnumFacing): Boolean =
        wrapped.onPlayerDestroyBlock(blockPos.unwrap())
