...
class GuiSlotWrapper(
    val wrapped: WrappedGuiSlot,
    mc: IMinecraft,
    width: Int,
    height: Int,
    topIn: Int,
    bottomIn: Int,
    slotHeightIn: Int
) : GuiSlot(mc.unwrap(), width, height, topIn, bottomIn, slotHeightIn) {
    ...
    override fun drawSlot(
        ...
        mouseYIn: Int,
        partialTicks: Float
    ) = wrapped.drawSlot(slotIndex, xPos, yPos, heightIn, mouseXIn, mouseYIn)
    ...
    public override fun elementClicked(slotIndex: Int, isDoubleClick: Boolean, mouseX: Int, mouseY: Int) =
        wrapped.elementClicked(slotIndex, isDoubleClick, mouseX, mouseY)
}