...
open class EntityImpl<T : Entity>(val wrapped: T) : IEntity {
    ...
    override fun setPositionAndUpdate(posX: Double, posY: Double, posZ: Double) =
        wrapped.setPositionAndUpdate(posX, posY, posZ)

    override fun rayTrace(range: Double, partialTicks: Float): IMovingObjectPosition? =
        wrapped.rayTrace(range, partialTicks)?.wrap()
    ...
    override fun setPositionAndRotation(
        ...
        rotationYaw: Float,
        rotationPitch: Float
    ) = wrapped.setPositionAndRotation(oldX, oldY, oldZ, rotationYaw, rotationPitch)
