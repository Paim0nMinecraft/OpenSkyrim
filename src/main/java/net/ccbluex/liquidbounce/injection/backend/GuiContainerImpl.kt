...
open class GuiContainerImpl<T : GuiContainer>(wrapped: T) : GuiScreenImpl<T>(wrapped), IGuiContainer {
    override fun handleMouseClick(slot: ISlot, slotNumber: Int, clickedButton: Int, clickType: Int) =
        (wrapped as IMixinGuiContainer).publicHandleMouseClick(
            slot.unwrap(),
            slotNumber,
            clickedButton,
            clickType.toClickType()
        )
