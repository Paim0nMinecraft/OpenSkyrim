...
open class WorldImpl<T : World>(val wrapped: T) : IWorld {
    ...
    override fun rayTraceBlocks(start: WVec3, end: WVec3): IMovingObjectPosition? =
        wrapped.rayTraceBlocks(start.unwrap(), end.unwrap())?.wrap()

    override fun rayTraceBlocks(start: WVec3, end: WVec3, stopOnLiquid: Boolean): IMovingObjectPosition? =
        wrapped.rayTraceBlocks(start.unwrap(), end.unwrap(), stopOnLiquid)?.wrap()
    ...
    ): IMovingObjectPosition? = wrapped.rayTraceBlocks(
        start.unwrap(),
        end.unwrap(),
        stopOnLiquid,
    ...
    override fun getBlockState2(blockPos: BlockPos): IBlockState = wrapped.getBlockState(blockPos)
    ...
    override fun getCollisionBoxes(bb: IAxisAlignedBB): Collection<IAxisAlignedBB> =
        WrappedCollection(wrapped.getCollisionBoxes(null, bb.unwrap()), IAxisAlignedBB::unwrap, AxisAlignedBB::wrap)
