...
class ItemStackImpl(val wrapped: ItemStack) : IItemStack {
    ...
    override fun addEnchantment(enchantment: IEnchantment, level: Int) =
        wrapped.addEnchantment(enchantment.unwrap(), level)

    // TODO Check if this workaround really works
    override fun getAttributeModifier(key: String): Collection<IAttributeModifier> = WrappedCollection(
        wrapped.getAttributeModifiers(EntityEquipmentSlot.MAINHAND)[key],
        IAttributeModifier::unwrap,
        AttributeModifier::wrap
    )
