...
open class GuiScreenImpl<T : GuiScreen>(wrapped: T) : GuiImpl<T>(wrapped), IGuiScreen {
    ...
    override fun superMouseReleased(mouseX: Int, mouseY: Int, state: Int) =
        (wrapped as GuiScreenWrapper).superMouseReleased(mouseX, mouseY, state)
    ...
    override fun superKeyTyped(typedChar: Char, keyCode: Int) =
        (wrapped as GuiScreenWrapper).superKeyTyped(typedChar, keyCode)

    override fun superHandleMouseInput() = (wrapped as GuiScreenWrapper).superHandleMouseInput()

    override fun superMouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) =
        (wrapped as GuiScreenWrapper).superMouseClicked(mouseX, mouseY, mouseButton)

    override fun superDrawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) =
        (wrapped as GuiScreenWrapper).superDrawScreen(mouseX, mouseY, partialTicks)
