...
class ChunkImpl(val wrapped: Chunk) : IChunk {
    ...
    override fun getEntitiesWithinAABBForEntity(
        thePlayer: IEntityPlayerSP,
        arrowBox: IAxisAlignedBB,
        collidedEntities: MutableList<IEntity>,
        nothing: Nothing?
    ) {
        return wrapped.getEntitiesWithinAABBForEntity(
            thePlayer.unwrap(),
            arrowBox.unwrap(),
            WrappedMutableList(collidedEntities, Entity::wrap, IEntity::unwrap),
            null
        )
    }
