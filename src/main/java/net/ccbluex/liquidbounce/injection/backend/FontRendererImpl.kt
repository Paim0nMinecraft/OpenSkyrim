...
class FontRendererImpl(val wrapped: FontRenderer) : IFontRenderer {
    ...
    override fun drawString(str: String, x: Float, y: Float, color: Int) =
        wrapped.drawString(str, x.toInt(), y.toInt(), color)

    override fun drawString(str: String, x: Float, y: Float, color: Int, shadow: Boolean) =
        wrapped.drawString(str, x, y, color, shadow)

    override fun drawCenteredString(text: String, x: Float, y: Float, color: Int) =
        drawString(text, x - getStringWidth(text) / 2F, y, color)

    override fun drawCenteredString(text: String, x: Float, y: Float, color: Int, shadow: Boolean) =
    ...
    override fun drawStringWithShadow(text: String, x: Int, y: Int, color: Int) =
        wrapped.drawStringWithShadow(text, x.toFloat(), y.toFloat(), color)
