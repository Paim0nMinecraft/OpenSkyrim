...
object ExtractedFunctionsImpl : IExtractedFunctions {
    ...
    override fun getModifierForCreature(heldItem: IItemStack?, creatureAttribute: IEnumCreatureAttribute): Float =
        EnchantmentHelper.getModifierForCreature(heldItem?.unwrap(), creatureAttribute.unwrap())

    override fun getObjectFromItemRegistry(res: IResourceLocation): IItem? =
        Item.REGISTRY.getObject(res.unwrap())?.wrap()

    override fun renderTileEntity(tileEntity: ITileEntity, partialTicks: Float, destroyStage: Int) =
        TileEntityRendererDispatcher.instance.render(tileEntity.unwrap(), partialTicks, destroyStage)
    ...
    override fun getEnchantmentByLocation(location: String): IEnchantment? =
        Enchantment.getEnchantmentByLocation(location)?.wrap()

    override fun getEnchantmentById(enchantID: Int): IEnchantment? = Enchantment.getEnchantmentByID(enchantID)?.wrap()

    override fun getEnchantments(): Collection<IResourceLocation> =
        WrappedCollection(Enchantment.REGISTRY.keys, IResourceLocation::unwrap, ResourceLocation::wrap)

    override fun getItemRegistryKeys(): Collection<IResourceLocation> =
        WrappedCollection(Item.REGISTRY.keys, IResourceLocation::unwrap, ResourceLocation::wrap)

    override fun getBlockRegistryKeys(): Collection<IResourceLocation> =
        WrappedCollection(Block.REGISTRY.keys, IResourceLocation::unwrap, ResourceLocation::wrap)
    ...
    override fun sessionServiceJoinServer(profile: GameProfile, token: String, sessionHash: String) =
        Minecraft.getMinecraft().sessionService.joinServer(profile, token, sessionHash)
    ...
    override fun scoreboardFormatPlayerName(scorePlayerTeam: ITeam?, playerName: String): String =
        ScorePlayerTeam.formatPlayerName(scorePlayerTeam?.unwrap(), playerName)
    ...
    override fun jsonToComponent(toString: String): IIChatComponent =
        ITextComponent.Serializer.jsonToComponent(toString)!!.wrap()
    ...
    override fun canAddItemToSlot(slotIn: ISlot, stack: IItemStack, stackSizeMatters: Boolean): Boolean =
        Container.canAddItemToSlot(slotIn.unwrap(), stack.unwrap(), stackSizeMatters)
