...
class BlockImpl(val wrapped: Block) : IBlock {
    ...
    override fun getSelectedBoundingBox(world: IWorld, blockState: IIBlockState, blockPos: WBlockPos): IAxisAlignedBB =
        AxisAlignedBBImpl(
            wrapped.getBoundingBox(blockState.unwrap(), world.unwrap(), blockPos.unwrap()).offset(blockPos.unwrap())
        )

    override fun getCollisionBoundingBox(world: IWorld, pos: WBlockPos, state: IIBlockState): IAxisAlignedBB? =
        wrapped.getCollisionBoundingBox(state.unwrap(), world.unwrap(), pos.unwrap())?.wrap()

    override fun canCollideCheck(state: IIBlockState?, hitIfLiquid: Boolean): Boolean =
        wrapped.canCollideCheck(state?.unwrap(), hitIfLiquid)
    ...
    override fun getPlayerRelativeBlockHardness(
        ...
        theWorld: IWorld,
        blockPos: WBlockPos
    ): Float = wrapped.getPlayerRelativeBlockHardness(
        theWorld.getBlockState(blockPos).unwrap(),
        thePlayer.unwrap(),
        theWorld.unwrap(),
        blockPos.unwrap()
    )
    ...
    override fun getMapColor(blockState: IIBlockState, theWorld: IWorldClient, bp: WBlockPos): Int =
        wrapped.getMapColor(blockState.unwrap(), theWorld.unwrap(), bp.unwrap()).colorValue
